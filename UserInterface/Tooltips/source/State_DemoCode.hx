...
class State_DemoCode extends FlxUIState
{
	...
	function makeFontDef(name:String, size:Int, alignment = null, isBold:Bool = true, color:FlxColor = FlxColor.WHITE, extension:String = ".ttf"):FontDef
	{
		...
		return new FontDef(name, extension, "assets/fonts/" + name + suffix + extension,
			new TextFormat(null, size, color, isBold, null, null, null, null, alignment));
	}
	...
	function makeByHand():Void
	{
		...
		b = addBtn("fancier", new FlxUIButton(b.x, b.y + b.height + 10, "Fancier"), "Fancier tooltip!", "This tooltip has a title AND a body.", null,
			{bodyWidth: 100});
