...
class Blob extends BaseState
{
	...
	function createBlob()
	{
		...
		while (i < 360)
		{
			...
			var circle:FlxNapeSprite = new FlxNapeSprite(FlxG.width / 2
				+ Math.cos(angle) * BLOB_RADIUS
				+ startXOffset,
				FlxG.height / 2
				+ Math.sin(angle) * BLOB_RADIUS
				+ startYOffset);
			circle.createCircularBody(CIRCLE_RADIUS);
			circle.body.allowRotation = false;
			circle.setBodyMaterial(1, 0.1, 2, 1, 0.001);
			circle.makeGraphic(CIRCLE_RADIUS * 2, CIRCLE_RADIUS * 2,
				0xFFFFFFFF); // Creates the dummie circle graphic used to add mouse events to this NapeSprite.
			circle.alpha = 0.01;
...
class Twinkle extends FlxNapeSprite
{
	...
	public function new()
	{
		...
		destinationJoint = new DistanceJoint(FlxNapeSpace.space.world, body, new Vec2(body.position.x, body.position.y), body.localCOM, 0, 0);
