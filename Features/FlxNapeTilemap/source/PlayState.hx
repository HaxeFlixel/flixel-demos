...
class PlayState extends FlxState
{
	...
	function initSpace():Void
	{
		...
		FlxNapeSpace.space.listeners.add(new PreListener(InteractionType.COLLISION, Constants.oneWayType, CbType.ANY_BODY, PhysUtil.oneWayHandler, 0, true));
	}
	...
	override public function create():Void
	{
		...
		var playerCtrl = new PlayerController(player.body, level.body, new PlayerControls([LEFT], [RIGHT], [UP]));
		...
		playerCtrl = new PlayerController(player.body, level.body, new PlayerControls([A], [D], [W]));
		point = level.spawnPoints[1];
		player.setPosition(point.x, point.y - player.height * 0.5);
		add(player);
		add(playerCtrl);

		var instructions = new FlxText(0, 50, FlxG.width, "Space to switch levels, Up / W to double-jump", 16);
