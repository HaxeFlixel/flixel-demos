...
class MenuState extends FlxState
{
	...
	{
		return [
			"Reset Global Seed", "Get Current Seed", "Generate Integers", "Generate Floats", "Normal Distribution Floats", "Weighted Coin Flips",
			"Generate Signs", "Weighted Pick", "Get Array Object", "Weighted Get Array Object", "Shuffle Array", "Generate Colors", "Test Accuracy"
		];
	}
	...
	override public function create():Void
	{
		...
		_display = new FlxText(10, 10, FlxG.width
			- 210,
			"Welcome to the FlxRandom demo!\nYour randomly generated seed is: "
			+ FlxG.random.initialSeed
	...
	function floatNormal():Void
	{
		...
		timer = Lib.getTimer();
		for (i in 0...TENMIL)
			dummyFloat = FlxG.random.floatNormal();
	...
	function floatRandom():Void
	{
		...
		timer = Lib.getTimer();
		for (i in 0...TENMIL)
			dummyFloat = FlxG.random.float();
		_display.text += "\nTime: " + (Lib.getTimer() - timer) + "ms using the newest method.";

		timer = Lib.getTimer();
		for (i in 0...TENMIL)
			dummyFloat = OldFlxRandom.floatRanged();
		_display.text += "\nTime: " + (Lib.getTimer() - timer) + "ms using the old, non-deterministic FlxG.random.";

		timer = Lib.getTimer();
		for (i in 0...TENMIL)
			dummyFloat = FlxRandom_3_3_4.floatRanged();
		_display.text += "\nTime: " + (Lib.getTimer() - timer) + "ms using FlxRandom from 3.3.4.";

		timer = Lib.getTimer();
		for (i in 0...TENMIL)
			dummyFloat = NonInlineFlxRandom.floatRanged();
		_display.text += "\nTime: " + (Lib.getTimer() - timer) + "ms using FlxRandom from 3.3.4 without inlining.";

		timer = Lib.getTimer();
		for (i in 0...TENMIL)
			dummyFloat = Math.random();
		_display.text += "\nTime: " + (Lib.getTimer() - timer) + "ms using Math.random().";
	}
	...
	function coinFlipsWeighted():Void
	{
		...
		timer = Lib.getTimer();
		for (i in 0...TENMIL)
			if (FlxG.random.bool(weight))
				heads++
			else
				tails++;

		_display.text = "Flipped a coin with a " + weight + "% chance of heads 10 million times. Got " + heads + " heads and " + tails + " tails. Time: "
			+ (Lib.getTimer() - timer) + "ms.";
	...
	function weightedPicks():Void
	{
		...
		for (i in array)
			arrayTotal += Std.int(i);
		for (i in 0...array.length)
			expectedResults.push(FlxMath.roundDecimal(100 * array[i] / arrayTotal, 2));

		_display.text += "\nExpected results (in percentages):\n" + expectedResults;

		timer = Lib.getTimer();
		for (i in 0...ONEMIL)
			results[FlxG.random.weightedPick(array)]++;
	...
	function getObjects():Void
	{
		...
		timer = Lib.getTimer();
		for (i in 0...ONEMIL)
			results[FlxG.random.getObject(array)]++;

		_display.text = "Got a random object from an array of 20 integers 1 million times. Time "
			+ (Lib.getTimer() - timer)
			+ "ms. Displaying results as a histogram.";
	...
	function getObjectsWeighted():Void
	{
		...
		for (i in 0...objectArray.length)
			expected.push(FlxMath.roundDecimal(100 * weightArray[i] / total, 2));
		...
		for (i in 0...ONEMIL)
			results[FlxG.random.getObject(objectArray, weightArray)]++;

		var percentResults:Array<Float> = [
			for (i in 0...objectArray.length) FlxMath.roundDecimal(100 * results[i] / ONEMIL, 2)
		];
